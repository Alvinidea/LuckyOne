...
class interTool():
    ...
    def initPageSign(self):
        # 导入所有组
        exc = excel(GROUPS)
        group_single = self.col_info(exc)
        # print(GROUPS)
        grow, gcol = exc.getRowsClosNum()
        # print(grow, gcol)
        # 按照行来存取
        ...
        retFinished = self.initPageSign_finished()

        return groups, group, retFinished, group_single


    def initPageSign_finished(self):
        fexc = excel(FINISHED)
    ...
    # 保存抽取的组 和 组员信息
    def initPageSign_saveSelect(self, LuckyGroup, LuckyMember):
...
class ShowMission_Zero(tk.Frame):
    ...
    def __init__(self, parent, root):
        ...
        self.nums, self.names, self.mrow = self.importInfo()
    ...
    def flush(self):
        self.nums, self.names, mrow = self.importInfo()
        index = 0
        self.textshow()
...
class StartPage_Sign(tk.Frame):
    ...
    def __init__(self, parent, root):
        ...
        self.LuckyGroup = 0
        self.LuckyMember = ''
        self.GroupInfo, self.Groups, self.Finished, self.group_single = self.importInfo()
    ...
    def importInfo(self):
        ...
        retFinished = self.finishedInfo()

        return groups, group, retFinished,group_single


    def finishedInfo(self):
        fexc = excel(FINISHED)
        frow, fcol = fexc.getRowsClosNum()
        retFinished = list()
...
class Pagetwo_Match(tk.Frame):
    ...
    def __init__(self, parent, root):
        ...
        self.speed = 0.02  # 滚动速度
        self.GroupsInfo, self.MissionsInfo, self.Groups, self.Missions, self.group_single, self.mission_single = self.matchInfo()
        # 组号 --- 题号
        self.Match = {}
        # 防止 开始按钮 多次点击
    ...
    def matchInfo(self):
        ...
        for index in range(1, grow+1):
            ...
            groups[tempG[0]] = tempG[0:gcol]
        group = exc.getColValues(1)[1:grow]
        group_single = self.gcol_info(exc)

        mexc = excel(MISSIONS)
        mission_single = self.mcol_info(mexc)
        mrow, mcol = mexc.getRowsClosNum()
        missions = {}
        for index in range(1, mrow + 1):
            tempM = mexc.getRowValues(index)
    ...
    def save_MatchInfo(self):
        exc = excel(MATCH, False)
        row = ['组号','组名', '题号', '题目']\
              +['成员'+str(index) for index in range(1, len(self.group_single['成员姓名'])+1)]\
              +['备注']
        # print(row)
        # row = ['组号','组名', '题号', '题目', '成员1', '成员2', '成员3', '成员4', '备注']
        exc.addRow(row)
        ...
        for Gnum in range(0, len(self.Groups)):
            title_col = self.mission_single['题目']
            MissionNum = self.Match[self.Groups[Gnum]]
            mname = self.MissionsInfo[MissionNum][title_col-1]

            # members = self.GroupsInfo[self.Groups[Gnum]][2:]
            names_col = self.group_single['成员姓名']
            row = self.GroupsInfo[self.Groups[Gnum]]
            gname = row[self.group_single['组名']-1]
            members = [row[index-1] for index in names_col]
            newRow = [self.Groups[Gnum], gname, MissionNum, mname]+members+[""]
            """newRow = [self.Groups[Gnum], self.Missions[Gnum], name,
                      members[0], members[1], members[2], members[3],
                      ""]"""
            exc.addRow(newRow)
